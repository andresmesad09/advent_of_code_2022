...
def check_sequence(seq: str) -> bool:
    list_clean = list(set(seq))
    if len(list_clean) == len(seq):
        return True
    return False


def first_part(env: str = "test"):
    file_name = "input_6_small.csv" if env == 'test' else "input_6.csv"
    input_path = Path().cwd() / "data" / file_name
    with input_path.open(mode='r', encoding='utf-8') as file:
        buffer = file.read()

    for i in range(len(buffer) - 4):
        condition = check_sequence(buffer[i:i+4])
        if condition:
            first_time = i+4
            break
    print(first_time)
...
def second_part(env: str = "test"):
    ...
    for i in range(len(buffer) - 14):
        condition = check_sequence(buffer[i:i+14])
        if condition:
            first_time = i+14
            break
    print(first_time)
